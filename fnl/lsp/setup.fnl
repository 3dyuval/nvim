  ...
  {:filetypes [:typescript :typescriptreact :javascript :javascriptreact :vue]
   ...
   {"textDocument/publishDiagnostics"
    (fn [err result ctx]
      (when (and result result.diagnostics)
        (set result.diagnostics
          (vim.tbl_filter
            (fn [d] (not (. ignored-diag-codes d.code)))
            result.diagnostics)))
    ...
    (fn [err result ctx config]
      ...
      (let [result (if (and result (= (. vim.bo ctx.bufnr :filetype) :vue))
                     (vim.tbl_filter
                       (fn [sym]
                         (let [n sym.name]
                           (not (or (= n :import)
                                    (= n :from)
                                    (= n :export)
                                    (vim.startswith n "import ")
                                    (vim.startswith n "script")))))
                       result)
                     result)]
        ((. vim.lsp.handlers "textDocument/documentSymbol") err result ctx config)))}})
