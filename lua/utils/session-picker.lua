  ...
  if not ok then
    ...
    for _, f in ipairs(Lib.get_session_list(root_dir)) do
      if (f and f.session_name) then
        ...
        if string.find(decoded_name, "/") then
          ...
          if string.find(session_path_part, "^/") then
            local branch_name
            if pipe_idx then
...
local function open()
  ...
  if (#items == 0) then
    ...
    local function _7_(picker, item)
    ...
    return snacks.picker({title = "Sessions", items = items, format = "text", on_confirm = _7_, preview = _9_})
  end
end
return {open = open}
