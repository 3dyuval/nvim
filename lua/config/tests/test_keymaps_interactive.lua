...
function M.display_results(conflicts, test_keymaps)
    ...
    for _, km in ipairs(test_keymaps) do
        table.insert(lines, string.format("- **%s** `%s` → `%s` (%s)", 
            km.mode or 'n', km.lhs, km.rhs, km.desc or 'no description'))
    end
    table.insert(lines, "")
    ...
    if #conflicts > 0 then
        ...
        for _, conflict in ipairs(conflicts) do
            ...
            local icon = conflict.severity == 'ERROR' and '🚨' or 
                        conflict.severity == 'WARNING' and '⚠️' or 'ℹ️'
            
            table.insert(lines, string.format("%s **Mode:** `%s`, **Key:** `%s` [%s]", 
                icon, conflict.mode, conflict.key, conflict.existing_type or conflict.conflict_type))
            table.insert(lines, string.format("  - **Existing:** `%s` - %s", 
                conflict.existing_rhs, conflict.existing_desc))
            table.insert(lines, string.format("  - **New:** `%s` - %s", 
