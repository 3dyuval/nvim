...
M.organize_imports = function()
		...
		if biome_available then
				...
				local cmd = {
						"biome", "check", "--write", "--unsafe", -- Enable unsafe fixes to remove unused imports
      ...
      filepath
				...
				vim.schedule(function() vtsls_cmd("typescript.removeUnusedImports", filepath) end
)
		end
end
...
M.organize_imports_and_fix = function()
		...
		vim.schedule(function() lsp_action("source.fixAll.ts") end
)
end
		...
		opts = {
				...
				formatters = {
						...
						rubocop = {
								command = "rubocop",
								args = {
										"--autocorrect-all", "--config",
          vim.fn.stdpath("config") .. "/formatters/rubocop.yml",
          "--format", "quiet", "--stderr", "--stdin",
          "$FILENAME"
								},
								stdin = true
						},
