...
local function _11_(_, opts)
  ...
  local function _12_(ev)
    ...
    for key, dir in pairs(dirs) do
      ...
      vim.keymap.set("n", key, _13_, {buffer = ev.buf, nowait = true, desc = "Window nav"})
    end
  ...
  return vim.api.nvim_create_user_command("Kulala", _16_, {nargs = "?", complete = _19_, desc = "Kulala HTTP client"})
end
return {"mistweaverco/kulala.nvim", ft = {"http", "rest"}, cmd = {"Kulala"}, keys = {{"<leader>crr", _1_, desc = "Run request"}, {"<leader>cro", _2_, ft = {"http", "rest"}, desc = "OpenAPI explorer"}, {"<leader>cr.", _3_, desc = "Scratchpad"}, {"<leader>cre", _4_, desc = "Select environment"}, {"<leader>crR", _5_, desc = "Replay last request"}, {"<leader>crc", _6_, desc = "Copy as cURL"}, {"<leader>crX", _7_, desc = "Clear cached files"}}, opts = {kulala_keymaps = {["Next tab"] = false, ["Previous tab"] = false}, openapi_panel_keymaps = {["Yank as HTTP"] = {"Y", _8_}, ["Edit try it out"] = false, ["Load from file"] = false, Refresh = false, ["Toggle fold"] = false}, ui = {pickers = {snacks = {layout = _9_}}}, global_keymaps = false}, config = _11_, dev = false}
